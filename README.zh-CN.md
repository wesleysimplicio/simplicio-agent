...
```bash
hermes              # 交互式 CLI — 开始对话
simplicio-agent model        # 选择 LLM 提供商和模型
simplicio-agent tools        # 配置启用的工具
simplicio-agent config set   # 设置单个配置项
simplicio-agent gateway      # 启动消息网关（Telegram、Discord 等）
simplicio-agent setup        # 运行完整设置向导（一次性配置所有内容）
simplicio-agent claw migrate # 从 OpenClaw 迁移（如果来自 OpenClaw）
simplicio-agent update       # 更新到最新版本
simplicio-agent doctor       # 诊断问题
```
...
| 操作 | CLI | 消息平台 |
|------|-----|----------|
| 开始对话 | `hermes` | 运行 `simplicio-agent gateway setup` + `simplicio-agent gateway start`，然后给机器人发消息 |
...
**首次安装时：** 安装向导（`simplicio-agent setup`）会自动检测 `~/.openclaw` 并在配置开始前提供迁移选项。

**安装后任意时间：**

```bash
simplicio-agent claw migrate              # 交互式迁移（完整预设）
simplicio-agent claw migrate --dry-run    # 预览将要迁移的内容
simplicio-agent claw migrate --preset user-data   # 仅迁移用户数据，不含密钥
simplicio-agent claw migrate --overwrite  # 覆盖已有冲突
```
...
使用 `simplicio-agent claw migrate --help` 查看所有选项，或使用 `openclaw-migration` 技能进行交互式代理引导迁移（含干运行预览）。
...
`simplicio-agent update`、托管 venv、lazy dependencies、gateway 和 docs tooling 使用的布局。
