...
class GatewayConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``simplicio-agent config set gateway.multiplex_profiles true``.
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # Both ``hermes`` and ``simplicio-agent`` persist nested
                # gateway.streaming settings.  Do not assume the section is
                # a mapping when reading older user configuration.
                streaming_cfg = (
                    gateway_section.get("streaming")
                    if isinstance(gateway_section, dict)
                    else None
                )
