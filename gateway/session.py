...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `simplicio-agent tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `simplicio-agent tools` AND the bot
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.hermes/state.db and are shown by `simplicio-agent sessions list` and "
