...
echo ""
echo -e "${CYAN}This script is a one-time INSTALL step${NC} — it built/linked the"
echo "simplicio binary, the neural memory DB, and the MCP server + plugin."
echo "It does NOT decide how the agent works turn-to-turn; that's DAILY USE:"
echo ""
echo "  1. executable native tools -> Simplicio-runtime first (\`simplicio ...\`)"
echo "  2. MCP is used only when the Runtime CLI is unavailable"
echo "  3. native fallback only as an explicit exception, when the runtime"
echo "     has a gap — report UNVERIFIED and log it so the runtime can grow"
echo ""
echo "Next steps:"
echo ""
if is_termux; then
