...
echo ""
echo "  1. read / search / analyze -> native Hermes tools first (orientation)"
echo "  2. mutate / validate / checkpoint -> Simplicio-runtime (\`simplicio ...\`)"
echo "  3. native fallback only as an explicit exception, when the runtime"
echo "     has a gap — log it so the runtime can grow to cover it"
echo ""
echo "Next steps:"
echo ""
if is_termux; then
