...
export const zh: Translations = {
  ...
  boot: {
    ready: 'Simplicio 桌面版已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败：${message}`,
    steps: {
      connectingGateway: '正在连接桌面网关',
      loadingSettings: '正在加载 Simplicio 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 Simplicio 桌面版…'
    },
    errors: {
      backgroundExited: 'Simplicio 后台进程已退出。',
      backgroundExitedDuringStartup: 'Simplicio 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Simplicio 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    updateHermes: '更新 Simplicio',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 Simplicio Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'Simplicio 正在等待你的回应。',
      turnDoneTitle: 'Simplicio 已完成',
      turnDoneBody: '回复已就绪。',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Simplicio 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 Simplicio 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'Simplicio 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'Simplicio 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '本轮以错误结束。'
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Simplicio 跟随系统设置。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 Simplicio 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 Simplicio，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 Simplicio 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'Simplicio 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'Simplicio 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'Simplicio 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Simplicio 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Simplicio 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Simplicio Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Simplicio 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 Simplicio 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 Simplicio 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'Simplicio Desktop 将使用已保存设置重新连接。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · Simplicio ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。Simplicio 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 Simplicio 为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  integrations: {
    title: '集成',
    subtitle: '将 Simplicio MCP 服务器部署到已安装的编辑器和智能体。',
    backendUnavailable: 'Simplicio 桥接不可用——无法从此窗口检查集成状态。',
    daemonTitle: 'Simplicio MCP 服务器',
    daemonAlwaysOn: '本地 MCP 服务器，始终在线',
    daemonRunning: '运行中',
    daemonStopped: '已停止',
    daemonPid: pid => `PID ${pid}`,
    daemonUptime: uptime => `已运行 ${uptime}`,
    daemonRestarts: count => `重启 ${count} 次`,
    daemonLastError: message => `最近错误：${message}`,
    editorsHeading: '编辑器与智能体',
    deployAll: '全部部署',
    deploying: '部署中...',
    deployedTitle: 'MCP 服务器已部署',
    deployedSummary: (registered, skipped) =>
      skipped > 0 ? `已注册 ${registered} 个，跳过 ${skipped} 个` : `已注册 ${registered} 个`,
    deployFailedTitle: '部署失败',
    deployResultRegisteredLabel: '已注册：',
    deployResultSkippedLabel: '已跳过：',
    deployResultNoneRegistered: '没有注册任何编辑器。',
    restartNote: '重启编辑器或智能体以加载服务器。',
    detectFailedTitle: '检测编辑器失败',
    noEditorsFound: '此设备上未检测到编辑器。',
  ...
  savings: {
    title: '代币经济',
    subtitle: '真实、有据可查的代币节省——能核实的标为已核实，不能核实的标为估算。',
    close: '关闭节省面板',
    refresh: '刷新',
    refreshing: '刷新中…',
    lastUpdated: time => `更新于 ${time}`,
    heroTotalSavedLabel: '累计节省代币',
    heroPctSavedLabel: '节省率',
    heroSpentLabel: '本期已花费',
    heroSpentHint: baseline => `基准值 ${baseline}`,
    evidenceSectionTitle: '证据',
    evidenceSectionDesc: '下方每一项都标注了已核实或估算——绝不以未标注方式呈现。',
    measuredLabel: '已核实',
    measuredTooltip: '由服务商或节省账本报告的真实用量。',
    estimatedLabel: '估算',
    estimatedTooltip: '启发式估算，非服务商报告的真实数字。',
    unknownProofLabel: '未标注',
    unknownProofTooltip: '该记录未携带 proof-kind 标签。',
    chartTitle: '累计节省趋势',
    perSessionTitle: '按会话',
    perSessionAggregatedNote: '此报告没有按会话拆分——改为显示原始账本事件。',
    noEventListDesc: '此报告仅有汇总数据——没有可显示的事件明细。',
    columnTimestamp: '时间',
    columnContext: '会话 / 仓库 / 模型',
    columnSpent: '花费',
    columnBaseline: '基准',
    columnSaved: '节省',
    columnProof: '证据',
    mcpUnknown: 'MCP 状态未知',
    mcpRunning: 'MCP 守护进程运行中',
    mcpRunningPid: pid => `MCP 守护进程运行中（pid ${pid}）`,
    mcpStopped: 'MCP 守护进程已停止',
    mcpStoppedNoDetail: '已停止 — 无错误报告',
    mcpRestarts: count => `重启 ${count} 次`,
    backendUnavailableTitle: '节省数据后端不可用',
    backendUnavailableDesc: '当前版本未暴露 window.simplicioSavings。请更新桌面应用，或在终端运行 `simplicio savings report --json` 直接查看账本。',
    errorTitle: '无法加载节省报告',
    retry: '重试',
    emptyTitle: '尚无节省记录',
    emptyDesc: '运行 `simplicio savings record --spent <N> --baseline <N> --proof-kind estimated` 记录第一条数据。',
    loading: '正在加载节省数据…',
    cockpit: {
      mcpLabel: 'MCP 服务器',
      llmLabel: '本地 LLM',
      ...
      byModelTitle: '按模型的节省',
      byProofTitle: '按证据类型的节省',
      sessionsTitle: '会话',
      sessionsDesc: '每次运行都是可审计的时间线：使用的命令、代币和可验证的哈希链。',
      eventsCount: count => `${count} 个事件`,
      noEvents: '此次运行没有记录事件。',
      savedShort: '节省',
      hashChainTooltip: '可验证哈希链（HBP）：prev -> this',
      sourceLabel: '来源：',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 Simplicio 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 Simplicio 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 Simplicio 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Simplicio 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `Simplicio ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 Simplicio 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Simplicio 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 Simplicio 指向该 URL。',
      ...
      weixin:
        '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Simplicio 会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 Simplicio 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Simplicio 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建定时任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。Simplicio 会运行它，并把结果发送到你选择的目的地。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend:
        '请更新 Simplicio 后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
      deleteConfirm: '这会从 Simplicio 中移除已保存的项目。文件、git 仓库和工作树保持不变。',
      startWork: '新建工作树',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 Simplicio…',
    placeholderReconnecting: '正在重新连接 Simplicio…',
    ...
    attachUrlDesc: 'Simplicio 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    stages: {
      ...
      pull: '马上完成…',
      pydeps: '收尾中…',
      update: '正在更新 Simplicio…',
      rebuild: '正在重新构建桌面应用…',
      restart: '正在重启 Simplicio…',
      done: '更新完成',
      manual: '从终端更新',
      guiSkew: '请更新桌面应用',
      error: '更新已暂停'
    ...
    unsupportedMessage: '此版本的 Simplicio 无法在应用内自行更新。',
    connectionRetry: '请检查网络连接后重试。',
    latestBody: '你正在运行最新版本。',
    latestBodyBackend: '后端正在运行最新版本。',
    allSetTitle: '已是最新',
    availableTitle: '有可用更新',
    availableBody: '新版 Simplicio 已可安装。',
    availableTitleBackend: '后端有可用更新',
    availableBodyBackend: '已连接的 Simplicio 后端有新版本可安装。',
    availableBodyNoChangelog: '已有新版本可用。此安装方式无法显示更新日志。',
    updateNow: '立即更新',
    maybeLater: '稍后再说',
    moreChanges: count => `另有 ${count} 项更改。`,
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 Simplicio，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 Simplicio 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 Simplicio 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody:
      'Simplicio 更新器会在自己的窗口中接管，并在完成后自动重新打开 Simplicio。更新期间请不要自行重新打开 Simplicio。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 Simplicio 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 Simplicio 会自动重新打开。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    oneTimeTitle: 'Simplicio 需要一次性安装',
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 Simplicio Agente',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 Simplicio CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 Simplicio 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。Simplicio 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 Simplicio Agente',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'Simplicio 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 Simplicio…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 Simplicio 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 Simplicio 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — Simplicio 会自动连接',
      loopback: '打开浏览器登录 — Simplicio 会自动连接',
    ...
    authorizeThere: '请在那里授权 Simplicio。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 Simplicio，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。Simplicio 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'Simplicio 正在重启...',
      askRestart: '让 Simplicio 重启服务器',
      lookingRestart: taskId => `Simplicio 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'Simplicio 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `Simplicio 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'Simplicio 无法重启服务器。',
      stillWorking: 'Simplicio 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    thread: {
      ...
      showEarlier: '显示更早的消息',
      loadingResponse: 'Simplicio 正在加载回复',
    ...
    approval: {
      gatewayDisconnected: 'Simplicio 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.hermes/config.yaml)。Simplicio 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'Simplicio 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'Simplicio 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 Simplicio 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 Simplicio 桌面版以保存图片',
