...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Simplicio Agent session")
        ...
        @tree.command(name="status", description="Show Simplicio Agent session status")
        ...
        @tree.command(name="stop", description="Stop the running Simplicio Agent")
        ...
        @tree.command(name="update", description="Update Simplicio Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Simplicio Agent gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Simplicio Agent session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Simplicio Agent in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Simplicio Agent skill",
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Simplicio Agent: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Simplicio Agent"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Simplicio Agent: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Simplicio Agent session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Simplicio Agent handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Simplicio Agent needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``simplicio-agent update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``simplicio-agent update`` prompts.
...
# is not in this process (e.g. ``simplicio-agent cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    print()
    print_info("📬 Home Channel: where Simplicio Agent delivers cron job results,")
