"""`simplicio-agent meet node ...` subcommand tree.

Wired into the existing ``simplicio-agent meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``simplicio-agent meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``simplicio-agent meet node ...``.
