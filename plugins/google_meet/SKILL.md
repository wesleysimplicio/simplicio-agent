...
simplicio-agent meet install                 # pip deps + Chromium (transcribe only)
simplicio-agent meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
simplicio-agent meet auth                    # optional; skips guest-lobby wait
simplicio-agent meet setup                   # preflight checks
```

`simplicio-agent meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
pip install playwright websockets && python -m playwright install chromium
simplicio-agent plugins enable google_meet
simplicio-agent meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
simplicio-agent meet node approve my-mac ws://<mac-ip>:18789 <token>
simplicio-agent meet node ping my-mac                   # confirm reachable
```

Run `simplicio-agent meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `simplicio-agent meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `simplicio-agent meet node approve`.
