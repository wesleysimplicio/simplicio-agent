...
│                                                                      │
│   NodeServer (from `simplicio-agent meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `simplicio-agent meet` CLI |
...
| `cli.py` | `simplicio-agent meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `simplicio-agent meet node {run,list,approve,remove,status,ping}` |
| `SKILL.md` | agent usage guide |

## Local quick start

```bash
simplicio-agent plugins enable google_meet
simplicio-agent meet install                                      # pip + Chromium
simplicio-agent meet setup                                        # preflight
simplicio-agent meet auth                                         # optional
simplicio-agent meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
simplicio-agent meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hermes/.env
simplicio-agent meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
simplicio-agent meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
simplicio-agent meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
...
simplicio-agent plugins enable google_meet
simplicio-agent meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
simplicio-agent meet node approve my-mac ws://<mac-ip>:18789 <token>
simplicio-agent meet node ping my-mac
