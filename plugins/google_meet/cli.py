...
Wires ``simplicio-agent meet <subcommand>``:
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``simplicio-agent meet`` argparse tree.
    ...
    try:
        ...
        def _node_unavailable(args):
            print(f"simplicio-agent meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: simplicio-agent meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: simplicio-agent meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: simplicio-agent meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "simplicio-agent meet join https://meet.google.com/abc-defg-hij"
        )
    else:
...
def _cmd_install(*, realtime: bool, assume_yes: bool) -> int:
    ...
    print("\ndone. verify with: simplicio-agent meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: simplicio-agent meet join <url>")
    return 0
...
def _cmd_stop() -> int:
    res = pm.stop(reason="simplicio-agent meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1


if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="simplicio-agent meet")
    register_cli(parser)
