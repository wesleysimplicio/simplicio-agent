# Contributing to Simplicio Agent

Thank you for contributing to Simplicio Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Simplicio Agent to work across supported platforms.
...
Related: #38284 covers the agent-side analog — Simplicio Agent itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Simplicio Agent install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `simplicio-agent skills browse` (labeled "official") and install it with `simplicio-agent skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `simplicio-agent skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into the Simplicio Agent plugin directory (`~/.hermes/plugins/`, the frozen compatibility path) or via a pip entry point.
...
- Integrate with `simplicio-agent memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Simplicio Agent ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.hermes/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build a Simplicio Agent Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)
...
Use this only if you intentionally do not want Simplicio Agent's managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `simplicio-agent` entrypoint from this venv; running the
...
```bash
# The standard installer already put `simplicio-agent` on PATH.
simplicio-agent doctor
simplicio-agent chat -q "Hello"
```

If you used the manual clone fallback, run `./simplicio-agent` from the checkout or
...
The user may skip setup and keep loading the skill. Simplicio Agent only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `simplicio-agent setup` or update `~/.hermes/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Simplicio Agent" is the secondary collaborator. If the contributor's commit shows "Simplicio Agent" as author because they used Simplicio Agent to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Simplicio Agent tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `simplicio-agent --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Simplicio Agent uses a data-driven skin system — no code changes needed to add a new skin.
...
Simplicio Agent runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the legacy Hermes compatibility wrapper (it has a stdlib fallback
...
Simplicio Agent has terminal access. Security matters.
...
2. **Test manually**: Run `simplicio-agent` and exercise the code path you changed
