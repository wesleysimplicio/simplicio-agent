...
## Hermes native first, then Simplicio-runtime
- Como o Simplicio Agent é um fork do Hermes, use primeiro as ferramentas nativas do Hermes para orientação, leitura, busca e análise quando isso reduzir atrito e custo.
- Depois, execute com o Simplicio-runtime tudo o que ele cobrir de forma determinística (map, memory, edit, validate, savings, etc.).
- Use `simplicio <subcommand>` via terminal quando MCP não estiver ativo.
- Use ferramentas nativas do Hermes para o que o Runtime ainda não cobrir, sem bloquear o fluxo.
- Se o Runtime não conseguir fazer algo, volte para as ferramentas nativas e registre a lacuna para evoluir o Runtime.
- Fluxo obrigatório: Hermes native read/search/orient -> Simplicio runtime execute/validate -> fallback nativo só para gaps.
