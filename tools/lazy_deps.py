...
remediation hint pointing at ``simplicio-agent tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # Dashboard (`simplicio-agent dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``simplicio-agent update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``simplicio-agent update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``simplicio-agent update``. Never raises; lazy-install failures
