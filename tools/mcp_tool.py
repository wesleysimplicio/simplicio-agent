    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "HERMES_HOME", os.path.join(os.path.expanduser("~"), ".simplicio_agent")
                )
            )
            candidates = [
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `simplicio-agent mcp login {server_name}` (or delete the tokens "
...
def is_mcp_tool_parallel_safe(tool_name: str) -> bool:
    ...
    Native names follow ``mcp__{server}__{tool}``.  Existing Simplicio
    configurations can still carry the historical ``mcp_{server}_{tool}``
    names during a rolling upgrade, so accept both shapes and rely on the
    exact registration provenance rather than attempting to parse either.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``simplicio-agent tools`` interactive configuration — connects to each
