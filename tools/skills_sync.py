...
# Marker file written by `simplicio-agent profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.simplicio-agent profile).
# When present in HERMES_HOME, sync_skills() is a no-op so neither the
# installer, `simplicio-agent update`, nor a direct sync re-injects bundled skills.
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # this skip, every `simplicio-agent update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `simplicio-agent skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `simplicio-agent skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `simplicio-agent update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``simplicio-agent update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``simplicio-agent update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``simplicio-agent skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `simplicio-agent skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    installer, ``simplicio-agent update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``simplicio-agent skills opt-out`` /
    ...
    try:
        if enabled:
            ...
            marker.write_text(
                ...
                "(`simplicio-agent skills opt-out`).\n"
            ...
            changed = existed
            message = (
                "Opted back in. The next `simplicio-agent update` (or `simplicio-agent skills "
