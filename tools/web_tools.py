...
Backend is selected during ``simplicio-agent tools`` setup (web.backend in config.yaml).
...
def _env_value(name: str) -> str:
    ...
    through Hermes' config/.env layer (``simplicio-agent config set``, ``simplicio-agent tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``simplicio-agent tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `simplicio-agent tools` repaint.
        try:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `simplicio-agent plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `simplicio-agent tools` to set one up."
                    ),
            }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`simplicio-agent plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
