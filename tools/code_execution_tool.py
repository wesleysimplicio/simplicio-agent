...
def _load_config() -> dict:
    ...
    ``simplicio-agent --tui`` where it is never used.  Read the lightweight raw config
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``simplicio-agent tools`` (e.g. web is turned off),
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.simplicio_agent/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
