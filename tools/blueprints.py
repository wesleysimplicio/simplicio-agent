...
`simplicio-agent skills publish` for sharing. No new source type, no new store, no new
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    to ``simplicio-agent skills publish`` to share. ``body`` is the plain-language
