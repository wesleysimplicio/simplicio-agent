...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``simplicio-agent -c`` / ``--resume`` (relaunch
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"SimplicioAgent/{_HERMES_VERSION}",
    }
...
class AIAgent:
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``simplicio-agent chat -q``. In that mode,
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        try:
            ...
            for msg in messages:
                ...
                pending_db_message_ids.append(msg_id)
            if pending_db_messages:
                self._session_db.append_messages(
                    session_id=self.session_id,
                    messages=pending_db_messages,
                )
                flushed_ids.update(pending_db_message_ids)
    ...
    def _coerce_api_error_detail(value: Any) -> str:
        ...
        if isinstance(value, dict):
            ...
            try:
                return _fast_dumps(value, ensure_ascii=False, sort_keys=True)
            except TypeError:
                return str(value)
    ...
    def _sanitize_hook_payload(cls, value: Any) -> Any:
        ...
        try:
            encoded = _fast_dumps(payload, ensure_ascii=False, default=str)
        except Exception:
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        if _dev:
            ...
            # `simplicio-agent logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``simplicio-agent auth add xai-oauth``) whose tokens belong to
