...
class Fact:
    ...
    def from_dict(cls, value: Any) -> "Fact":
        ...
        return cls(
            text=_clean_text(value.get("text", value.get("statement", value.get("fact")))),
...
class Inference:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "basis", tuple(_clean_text(v) for v in self.basis if _clean_text(v)))
    ...
    def from_dict(cls, value: Any) -> "Inference":
        ...
        if isinstance(basis, str):
            basis = (basis,)
        return cls(
            text=_clean_text(value.get("text", value.get("conclusion", value.get("inference")))),
...
class Evidence:
    ...
    def to_dict(self) -> dict[str, Any]:
        result: dict[str, Any] = {"reference": self.reference, "verified": self.verified}
        if self.kind:
    ...
    def from_dict(cls, value: Any) -> "Evidence":
        ...
        return cls(
            reference=_clean_text(value.get("reference", value.get("ref", value.get("evidence")))),
...
class WatcherRequirement:
    ...
    def from_dict(cls, value: Any) -> "WatcherRequirement":
        ...
        return cls(
            name=_clean_text(value.get("name", value.get("watcher", value.get("requirement")))),
            ...
            recomputed=(bool(value["recomputed"]) if value.get("recomputed") is not None else None),
        )
...
class GoalContract:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "acceptance_criteria", tuple(_clean_text(v) for v in self.acceptance_criteria if _clean_text(v)))
        object.__setattr__(self, "facts", tuple(v if isinstance(v, Fact) else Fact.from_dict(v) for v in self.facts))
        object.__setattr__(self, "inferences", tuple(v if isinstance(v, Inference) else Inference.from_dict(v) for v in self.inferences))
        object.__setattr__(self, "open_questions", tuple(v if isinstance(v, OpenQuestion) else OpenQuestion.from_dict(v) for v in self.open_questions))
        object.__setattr__(self, "evidence", tuple(v if isinstance(v, Evidence) else Evidence.from_dict(v) for v in self.evidence))
        object.__setattr__(self, "watchers", tuple(v if isinstance(v, WatcherRequirement) else WatcherRequirement.from_dict(v) for v in self.watchers))
        ...
        if self.state is GoalState.COMPLETED_VERIFIED and not self.can_complete_verified():
    ...
    def is_complete(self) -> bool:
        return self.state in {GoalState.COMPLETED_VERIFIED, GoalState.COMPLETED_UNVERIFIED}
    ...
    def can_complete_verified(self) -> bool:
        return self.has_verified_evidence and self.required_watchers_satisfied and not any(q.blocking for q in self.open_questions)

    def _updated(self, **changes: Any) -> "GoalContract":
        changes.setdefault("updated_at_ns", time.time_ns())
        return replace(self, **changes)

    def add_fact(self, fact: Fact | str, *, source: str = "", confidence: Optional[float] = None) -> "GoalContract":
        item = fact if isinstance(fact, Fact) else Fact(fact, source=source, confidence=confidence)
        return self._updated(facts=self.facts + (item,))

    def add_inference(self, inference: Inference | str, *, basis: Iterable[str] = (), confidence: Optional[float] = None) -> "GoalContract":
        item = inference if isinstance(inference, Inference) else Inference(inference, tuple(basis), confidence)
        return self._updated(inferences=self.inferences + (item,))

    def add_open_question(self, question: OpenQuestion | str, *, blocking: bool = False) -> "GoalContract":
        item = question if isinstance(question, OpenQuestion) else OpenQuestion(question, blocking=blocking)
        return self._updated(open_questions=self.open_questions + (item,))

    def add_evidence(self, evidence: Evidence | str, *, kind: str = "", verified: bool = True, watcher_id: str = "") -> "GoalContract":
        item = evidence if isinstance(evidence, Evidence) else Evidence(evidence, kind=kind, verified=verified, watcher_id=watcher_id)
        return self._updated(evidence=self.evidence + (item,))

    record_evidence = add_evidence

    def add_watcher(self, watcher: WatcherRequirement | str, *, required: bool = True) -> "GoalContract":
        item = watcher if isinstance(watcher, WatcherRequirement) else WatcherRequirement(watcher, required=required)
        return self._updated(watchers=self.watchers + (item,))

    require_watcher = add_watcher

    def satisfy_watcher(self, name: str, *, receipt: str = "", recomputed: bool = True) -> "GoalContract":
        updated = tuple(
            replace(w, satisfied=True, receipt=receipt or w.receipt, recomputed=recomputed) if w.name == name else w
    ...
    def transition(self, state: GoalState | str, *, reason: str = "") -> "GoalContract":
        ...
        if target is self.state:
            return self
        allowed = {
            GoalState.ACTIVE: {GoalState.PAUSED, GoalState.BLOCKED, GoalState.COMPLETED_VERIFIED, GoalState.COMPLETED_UNVERIFIED, GoalState.FAILED, GoalState.CANCELLED},
            GoalState.PAUSED: {GoalState.ACTIVE, GoalState.BLOCKED, GoalState.CANCELLED},
        ...
        if target is GoalState.COMPLETED_VERIFIED and not self.can_complete_verified():
            raise VerificationRequiredError("completed_verified requires verified evidence, satisfied watchers, and no blocking open questions")
        if self.state not in allowed or target not in allowed[self.state]:
            raise InvalidGoalTransition(f"invalid goal transition {self.state.value!r} -> {target.value!r}")
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "GoalContract":
        ...
        version = data.get("schema_version", data.get("version", GOAL_CONTRACT_SCHEMA_VERSION))
        if version != GOAL_CONTRACT_SCHEMA_VERSION:
            raise ValueError(f"unsupported schema_version {version!r}; expected {GOAL_CONTRACT_SCHEMA_VERSION!r}")
        objective = _clean_text(data.get("objective"))
        criteria = tuple(_clean_text(v) for v in data.get("acceptance_criteria", data.get("acceptance", ())) if _clean_text(v))
        expected_objective_hash = data.get("objective_hash")
        expected_ac_hash = data.get("acceptance_criteria_hash", data.get("ac_hash"))
        if expected_objective_hash and expected_objective_hash != _canonical_hash(objective):
            raise GoalContractError("objective hash does not match serialized objective")
        if expected_ac_hash and expected_ac_hash != _canonical_hash(list(criteria)):
            raise GoalContractError("acceptance criteria hash does not match serialized criteria")
        return cls(
            objective=objective,
            acceptance_criteria=criteria,
            facts=tuple(Fact.from_dict(v) for v in data.get("facts", ())),
            inferences=tuple(Inference.from_dict(v) for v in data.get("inferences", ())),
            open_questions=tuple(OpenQuestion.from_dict(v) for v in data.get("open_questions", data.get("questions", ()))),
            evidence=tuple(Evidence.from_dict(v) for v in data.get("evidence", ())),
            watchers=tuple(WatcherRequirement.from_dict(v) for v in data.get("watchers", data.get("watcher_requirements", ()))),
...
__all__ = [
    "GOAL_CONTRACT_SCHEMA", "GOAL_CONTRACT_SCHEMA_VERSION", "GOAL_CONTRACT_VERSION",
    "GoalState", "GoalStatus", "TerminalState", "TERMINAL_STATES",
    "GoalContractError", "InvalidGoalTransition", "VerificationRequiredError",
    "Fact", "Inference", "OpenQuestion", "Evidence", "WatcherRequirement",
    "GoalFact", "GoalInference", "GoalOpenQuestion", "StructuredFact", "StructuredInference",
    "VerificationEvidence", "WatcherGate",
    "GoalContract",
]
