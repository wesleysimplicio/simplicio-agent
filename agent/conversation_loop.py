        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if (
                    ...
                    print(
                        f"{agent.log_prefix}     • Re-authenticate: simplicio-agent auth add nous"
                    )
                    print(
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(
                            f"{agent.log_prefix}   Run `simplicio-agent doctor` for credential-chain diagnostics, or"
                        )
                        print(
                    ...
                    print(
                        f'{agent.log_prefix}     • Legacy cleanup: simplicio-agent config set ANTHROPIC_TOKEN ""'
                    )
                    print(
                        f'{agent.log_prefix}     • Clear stale keys: simplicio-agent config set ANTHROPIC_API_KEY ""'
                    )
                    ...
                    if (
                        ...
                        if (
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}      2. Then run `simplicio-agent auth` to re-authenticate.",
                                    force=True,
                                )
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `simplicio-agent model`.",
                                    force=True,
                                )
                            else:  # nous
                                agent._vprint(
                            ...
                            agent._vprint(
                                f"{agent.log_prefix}      • Is the key valid? Run: simplicio-agent setup",
                                force=True,
                            )
                            agent._vprint(
