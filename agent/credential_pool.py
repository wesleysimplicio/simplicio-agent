...
def _write_through_provider_state_to_global_root(
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".simplicio_agent" / "auth.json"
            try:
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``simplicio-agent model`` / ``simplicio-agent auth`` which
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``simplicio-agent auth status``) would land on
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `simplicio-agent model` / `simplicio-agent auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `simplicio-agent model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``simplicio-agent auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `simplicio-agent auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `simplicio-agent auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `simplicio-agent setup` ("Claude
        ...
        # that `simplicio-agent setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `simplicio-agent auth add nous --label <name>`).  Fall back to the
        ...
        # standard `simplicio-agent auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `simplicio-agent auth remove openai-codex` marks
        ...
        # via `simplicio-agent auth openai-codex`.
        ...
        # When the user logs in via ``simplicio-agent model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``simplicio-agent auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `simplicio-agent auth remove <provider> <N>` for an
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `simplicio-agent auth` command that confirmed the source is gone).
