...
``simplicio-agent -p <profile>`` joins the same board as the dispatcher that
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``simplicio-agent gateway run --replace`` /
...
def create_task(
    ...
    *,
    ...
    each name to ``simplicio-agent --skills ...``. Use this to pin a task to a
    ...
    # `simplicio-agent --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``simplicio-agent -p <assignee>`` which fails
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``simplicio-agent -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `simplicio-agent -p <name>` the
    ...
    # back to Path.home() / ".simplicio_agent" (the DEFAULT profile root), ignoring the
    ...
    # that even when the worker activates a profile (`simplicio-agent -p <name>`
    ...
    # `simplicio-agent -p <assignee>` activates the profile, but the env var is
