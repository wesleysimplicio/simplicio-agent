...
configured for "cli" in `simplicio-agent tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `simplicio-agent chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"simplicio-agent -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "simplicio-agent -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"simplicio-agent -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "simplicio-agent -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "simplicio-agent -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "simplicio-agent -z: --provider requires --model (or HERMES_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"simplicio-agent -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("simplicio-agent -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``simplicio-agent -z`` / oneshot mode.
