...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``simplicio-agent skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``simplicio-agent skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``simplicio-agent skills list`` and
...
#   - moa: high-cost slash mode, available through /simplicio-agent moa to avoid
