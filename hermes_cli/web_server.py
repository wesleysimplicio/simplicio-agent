...
try:
    ...
    # running `simplicio-agent dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``simplicio-agent gateway run`` — but the
    desktop app spawns a ``simplicio-agent dashboard`` backend, not a gateway, so a cron
...
app = FastAPI(title="Simplicio Agent", version=__version__, lifespan=_lifespan)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Simplicio Agent (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``simplicio-agent model`` custom flow collects. Honored only on the main slot for
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``simplicio-agent update``.
    ...
    the dashboard's ``simplicio-agent update`` button is the correct update path and
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``simplicio-agent status`` and the
...
# `simplicio-agent curator` exposes.
# ---------------------------------------------------------------------------
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``simplicio-agent gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``simplicio-agent gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``simplicio-agent gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``simplicio-agent update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Simplicio Agent updates are managed outside this dashboard in "
    ...
    try:
        ...
        _log.exception("Failed to spawn simplicio-agent update")
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/hermes/update`` actually runs ``simplicio-agent update``.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `simplicio-agent model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `simplicio-agent model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``simplicio-agent model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``simplicio-agent model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `simplicio-agent model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `simplicio-agent model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Simplicio Agent from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Simplicio Agent to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Simplicio Agent from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Simplicio Agent to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Simplicio Agent in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Simplicio Agent through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Simplicio Agent via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Simplicio Agent through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Simplicio Agent to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Simplicio Agent inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Simplicio Agent to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Simplicio Agent through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Simplicio Agent to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Simplicio Agent to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Simplicio Agent as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in simplicio-agent gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in simplicio-agent gateway setup",
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"SimplicioAgentDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Simplicio Agent").strip() or "Simplicio Agent"
...
# the canonical ``simplicio-agent auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Simplicio Agent PKCE ({_HERMES_OAUTH_FILE})",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "simplicio-agent auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "simplicio-agent auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "simplicio-agent auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "simplicio-agent auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "simplicio-agent auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "simplicio-agent auth add anthropic",
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Simplicio Agent — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``simplicio-agent model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `simplicio-agent model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"simplicio-agent auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `simplicio-agent model` picker; _OAUTH_OVERRIDES supplies per-provider
...
# Guarded so simplicio-agent web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``simplicio-agent auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
# the tokens exactly like `simplicio-agent auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `simplicio-agent auth add xai-oauth`'s credential-pool insert.
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``simplicio-agent auth add minimax-oauth``.
    """
    from hermes_cli.auth import (
        _minimax_poll_token,
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `simplicio-agent sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `simplicio-agent sessions prune`)."""
...
# servers managed here show up under `simplicio-agent mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `simplicio-agent mcp catalog` / `simplicio-agent mcp install` read.  ``profile`` scopes
...
# spawn the real `simplicio-agent gateway <verb>` so behaviour matches the CLI exactly.
...
# `simplicio-agent memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `simplicio-agent memory setup` to configure a new one.",
            )
...
# Human-readable labels for each hub source id (matches `simplicio-agent skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Nous)",
    "hermes-index": "Simplicio Agent Index",
    "skills-sh": "skills.sh",
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`simplicio-agent -p <name> skills install`)
...
def _profile_setup_command(name: str) -> str:
    ...
    return "simplicio-agent setup" if name == "default" else f"{name} setup"
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``simplicio-agent profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``simplicio-agent profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``simplicio-agent profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``simplicio-agent tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``simplicio-agent tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``simplicio-agent tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``simplicio-agent tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``simplicio-agent tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``simplicio-agent -p <profile> tools post-setup``.
...
#
# The endpoint spawns the same ``simplicio-agent --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``simplicio-agent --tui`` would run.  Tests monkeypatch this
    ...
    — the same propagation ``simplicio-agent -p <name>`` performs. The in-process
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Simplicio Agent inside WSL2 to use the dashboard's /chat "
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Simplicio Teal",         "description": "Classic dark teal — the canonical Simplicio Agent look"},
    {"name": "default-large", "label": "Simplicio Teal (Large)", "description": "Simplicio Teal with bigger fonts and roomier spacing"},
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"simplicio-agent auth {name}"
                        break
            except Exception:
                pass
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `simplicio-agent dashboard register` (Nous Portal) or "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"  Simplicio Agent Web UI → http://{host}:{actual_port}")
