...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'simplicio-agent doctor' for details."
        )
        return None
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'simplicio-agent model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.hermes/.env or run 'simplicio-agent model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'simplicio-agent model')."
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `simplicio-agent auth list`). If the key is
