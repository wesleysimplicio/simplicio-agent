"""``simplicio-agent mcp`` subcommand parser.
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Simplicio Agent as an MCP server",
        description=(
            "Manage MCP server connections and run Simplicio Agent as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'simplicio-agent mcp add' to connect to a new server, or\n"
            "'simplicio-agent mcp serve' to expose Simplicio Agent conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Simplicio Agent as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `simplicio-agent mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `simplicio-agent mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `simplicio-agent mcp install n8n`)",
    )
