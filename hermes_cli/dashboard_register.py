"""``simplicio-agent dashboard register`` — register a self-hosted dashboard OAuth client.
...
This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     ...
     "run `simplicio-agent setup`" hint when the user isn't logged in.
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``simplicio-agent dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `simplicio-agent auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    print(
        ...
        "  `simplicio-agent dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    simplicio-agent dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `simplicio-agent dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `simplicio-agent setup` (or `simplicio-agent auth add nous`) first, then retry.")
        else:
