...
* ``simplicio-agent profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``simplicio-agent skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``simplicio-agent profile``, not a parallel tree):

    simplicio-agent profile install <source> [--name N] [--alias] [--force] [--yes]
    simplicio-agent profile update  <name>  [--force-config] [--yes]
    simplicio-agent profile info    <name>
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``simplicio-agent profile update`` can
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `simplicio-agent profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`simplicio-agent profile install <source> --name {canon} --force`."
        )
