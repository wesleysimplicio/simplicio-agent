...
Add, remove, or reorder entries here — both `simplicio-agent setup` and
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `simplicio-agent model`
...
# this list:  simplicio-agent model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `simplicio-agent model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `simplicio-agent model` TUI
...
# slug as a top-level row in the interactive `simplicio-agent model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``simplicio-agent model``, the
...
# path. The *interactive* default (GUI onboarding / ``simplicio-agent model``) uses the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``simplicio-agent auth add openai-codex`` without
    ``simplicio-agent model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``simplicio-agent model`` picker offers first. For metered aggregators
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``simplicio-agent model``, ``/model``, etc.).
    """
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``simplicio-agent auth add copilot`` and by ``_seed_from_env``
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``simplicio-agent model`` shows.
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `simplicio-agent moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
