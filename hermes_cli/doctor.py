...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``simplicio-agent --version`` reporting a stale version while
...
def run_doctor(args):
    ...
    # Handle `simplicio-agent doctor --ack <id>` as a fast path. Persist the ack and
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`simplicio-agent doctor --ack {hit.advisory.id}`."
                )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'simplicio-agent setup')",
                            (
        ...
        try:
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `simplicio-agent setup` run)",
                )
                if should_fix:
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                "# Simplicio Agent Persona\n\n"
                "<!-- Edit this file to customize how Simplicio Agent communicates. -->\n\n"
                "You are Simplicio Agent, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    try:
        ...
        if _rt.satisfied:
            ...
            _fail_and_issue(
                ...
                "Update the simplicio kernel: run 'simplicio-agent doctor --fix' "
            ...
            _fail_and_issue(
                ...
                "Install the simplicio kernel: run 'simplicio-agent doctor --fix' "
                "(release download or sibling cargo build)",
                issues,
            )
            if _rt.detail:
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `simplicio-agent update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `simplicio-agent doctor` invocation.
    ...
    def _probe_apikey_provider(
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``simplicio-agent doctor``.
    ...
    # never the legitimate source for `simplicio-agent doctor`.
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                _fail_and_issue(
                    ...
                    "No Honcho API key — run 'simplicio-agent memory setup'",
                    issues,
                )
            else:
                from plugins.memory.honcho.client import (
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run simplicio-agent memory setup)",
