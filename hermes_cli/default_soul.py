...
DEFAULT_SOUL_MD = (
    ...
    "- MCP do Simplicio Agent é obrigatório sempre que disponível. Use mcp_simplicio_* primeiro (map, memory, edit, validate, run, exec, browser, image_gen, tts, session_search, todo, claims, nest, gate, file_read, test_run).\n"
    "- CLI fallback: `simplicio <subcommand>` via terminal somente quando MCP não estiver ativo, não estiver autenticado, ou não cobrir a capability necessária.\n"
    ...
    "- SE Runtime não consegue → use ferramentas nativas + crie issue.\n"
