...
DEFAULT_SOUL_MD = (
    ...
    "\n"
    "## Simplicio Runtime — CLI first, MCP fallback\n"
    "- Simplicio-runtime-first: toda ferramenta nativa executável tenta o "
    "Simplicio-runtime primeiro; MCP só entra quando a CLI está indisponível.\n"
    "- Fallback nativo é explícito e deve carregar `UNVERIFIED| runtime capability gap`.\n"
    "- O Agent opera ATRAVÉS do Runtime. Toda ação é Runtime.\n"
    "- Runtime MCP tools remain available as the transport fallback (map, memory, edit, validate, run, exec, browser, image_gen, tts, session_search, todo, claims, nest, gate, file_read, test_run).\n"
    "- CLI is the primary Runtime transport: `simplicio <subcommand>`.\n"
