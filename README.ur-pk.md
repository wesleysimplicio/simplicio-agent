...
simplicio-agent model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
simplicio-agent tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
simplicio-agent config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
simplicio-agent gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
simplicio-agent setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
simplicio-agent claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
simplicio-agent update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
simplicio-agent doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `hermes`                                      | `simplicio-agent gateway setup` اور `simplicio-agent gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`simplicio-agent setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
simplicio-agent claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
simplicio-agent claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
simplicio-agent claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
simplicio-agent claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
تمام آپشنز دیکھنے کے لیے `simplicio-agent claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔
