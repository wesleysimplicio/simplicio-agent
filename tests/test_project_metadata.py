...
def test_project_name_is_canonical():
    pyproject_path = Path(__file__).resolve().parents[1] / "pyproject.toml"
    with pyproject_path.open("rb") as handle:
        project = tomllib.load(handle)["project"]
    assert project["name"] == "simplicio-agent"
...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, (
        "[matrix] extra must still exist for explicit `pip install simplicio-agent[matrix]`"
    )
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            spec for spec in all_extra_specs if f"simplicio-agent[{extra}]" in spec
        ]
...
def test_dev_extra_excluded_from_all():
    ...
    assert not any(
        spec == "simplicio-agent[dev]" for spec in optional_dependencies["all"]
    )
...
def test_nemo_relay_extra_uses_supported_official_distribution_range():
    ...
    assert not any(
        spec == "simplicio-agent[nemo-relay]" for spec in optional_dependencies["all"]
    )
