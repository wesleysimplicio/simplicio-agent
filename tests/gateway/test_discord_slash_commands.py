...
async def test_auto_create_thread_falls_back_to_hermes_when_only_mentions(adapter):
    ...
    empty — fall back to the "Simplicio Agent" default rather than ''.

    (Simplicio rebrand renamed this fallback from "Hermes".)
    """
    thread = SimpleNamespace(id=999, name="Simplicio Agent")
    message = SimpleNamespace(
...
async def test_auto_create_thread_falls_back_to_seed_message(adapter):
    ...
    result = await adapter._auto_create_thread(message)
    assert result is thread
    message.channel.send.assert_awaited_once_with("🧵 Thread created by Simplicio Agent: **Hello**")
    seed_message.create_thread.assert_awaited_once_with(
        name="Hello",
        auto_archive_duration=1440,
