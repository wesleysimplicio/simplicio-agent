...
Regression guard for the Windows ``simplicio-agent gateway restart`` silent-outage bug:
...
ACCEPT = [
    ...
    "hermes-gateway.exe",
    "simplicio-agent gateway",          # bare `simplicio-agent gateway` defaults to run
    "simplicio-agent gateway run",
    # profile selector AFTER the `gateway` token (argv is profile-position
    # agnostic — _apply_profile_override strips --profile/-p anywhere)
    "simplicio-agent gateway --profile work run",
    "python -m hermes_cli.main gateway -p work run",
    "simplicio-agent gateway --profile=work run",
    # a profile literally NAMED "gateway"
    "simplicio-agent -p gateway gateway run",
