...
# spawned with `simplicio-agent -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Worker's perspective (profile activated by `simplicio-agent -p coder`).
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "Simplicio Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
