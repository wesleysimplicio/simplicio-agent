...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``simplicio-agent auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``simplicio-agent auth add openai-codex``.
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `simplicio-agent auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``simplicio-agent auth add openai-codex`` for accounts B and C
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``simplicio-agent auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``simplicio-agent auth add openai-codex`` as a workaround when there was no
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run simplicio-agent auth".
    """
    from hermes_cli.auth import (
    ...
    assert "simplicio-agent auth" not in rendered
