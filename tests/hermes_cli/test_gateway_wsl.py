...
class TestSupportsSystemdServicesWSL:
    ...
    def test_wsl_with_systemd(self, monkeypatch):
        ...
        # supports_systemd_services() also gates on `systemctl` being on
        # PATH, which a real Linux/WSL box has but a macOS/CI-container dev
        # host running this test suite may not — mock it so this test
        # exercises the WSL-systemd branch under test, not PATH lookup.
    ...
    def test_native_linux(self, monkeypatch):
        ...
        # See test_wsl_with_systemd: mock `systemctl` on PATH so this
        # exercises the native-Linux branch, not a PATH-lookup miss on a
        # non-Linux dev host.
        monkeypatch.setattr(gateway.shutil, "which", lambda name: "/usr/bin/systemctl")
        assert gateway.supports_systemd_services() is True
...
class TestGatewayCommandWSLMessages:
    ...
    def test_install_wsl_no_systemd(self, monkeypatch, capsys):
        """simplicio-agent gateway install on WSL without systemd shows guidance."""
        monkeypatch.setattr(gateway, "is_linux", lambda: True)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
    ...
    def test_start_wsl_no_systemd(self, monkeypatch, capsys):
        """simplicio-agent gateway start on WSL without systemd shows guidance."""
        monkeypatch.setattr(gateway, "is_linux", lambda: True)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
    ...
    def test_status_wsl_running_manual(self, monkeypatch, capsys):
        """simplicio-agent gateway status on WSL with manual process shows WSL note."""
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
    ...
    def test_status_wsl_not_running(self, monkeypatch, capsys):
        """simplicio-agent gateway status on WSL with no process shows WSL start advice."""
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "is_termux", lambda: False)
        monkeypatch.setattr(gateway, "is_wsl", lambda: True)
