...
class TestGetHermesHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert home == Path.home() / ".simplicio_agent"
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``simplicio-agent logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert "simplicio-agent config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert "simplicio-agent config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            assert "simplicio-agent config:" in after_edit, "edited file should re-warn"
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `simplicio-agent setup`) lets a
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `simplicio-agent setup` surface it (previously only Honcho was listed, leaving
