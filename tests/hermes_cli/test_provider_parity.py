...
the SAME provider universe as ``simplicio-agent model`` (the CLI/TUI picker).
...
def test_every_hermes_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `simplicio-agent model` universe."""
    ...
    assert not missing, (
        "providers shown in `simplicio-agent model` but not configurable in the desktop "
