"""Tests for `simplicio-agent memory setup [provider]` routing.
...
``simplicio-agent memory setup honcho``) without the interactive picker — which matters
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "simplicio-agent memory setup" in out
