...
Phase 4 wires `simplicio-agent -p <profile> gateway start/stop` through the s6
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"simplicio-agent profile create {PROFILE}")
    ...
    r = _sh(container_name, f"simplicio-agent -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"simplicio-agent -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
...
def test_profile_delete_stops_gateway(
    ...
    _sh(container_name, f"simplicio-agent profile create {PROFILE}")
    _sh(container_name, f"simplicio-agent -p {PROFILE} gateway start", timeout=60)
    time.sleep(3)

    r = _sh(
        container_name,
        f"simplicio-agent profile delete {PROFILE} --yes",
        timeout=30,
    )
