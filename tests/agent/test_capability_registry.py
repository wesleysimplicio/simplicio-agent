...
def capability(name: str, **kwargs) -> Capability:
    ...
    metadata_keys = {"version", "source", "license", "platforms", "health", "risk", "determinism", "cost", "health_detail"}
    metadata = CapabilityMetadata(**{key: values.pop(key) for key in tuple(values) if key in metadata_keys})
    return Capability(name, metadata, **values)


def test_metadata_contains_all_routing_dimensions() -> None:
    item = capability("local", health=Health.DEGRADED, risk=Risk.MEDIUM, determinism=Determinism.REPEATABLE, cost=0.25)
    assert item.to_dict() == {
        "name": "local", "version": "1.0.0", "source": "test", "license": "MIT",
        "platforms": ["*"], "health": "degraded", "risk": "medium",
        "determinism": "repeatable", "cost": 0.25, "health_detail": "",
        "fallback": [], "repair_actions": [], "enabled": True,
    }
...
def test_missing_fallback_is_reported_deterministically() -> None:
    registry = CapabilityRegistry([capability("requested", health=Health.UNHEALTHY, fallback=("missing",))])
    result = registry.route("requested")
...
def test_platform_and_cost_filters() -> None:
    registry = CapabilityRegistry([capability("linux", platforms=("linux",), cost=2), capability("cheap", platforms=("linux",), cost=1)])
    assert registry.route("linux", platform="win32").reason is ReasonCode.PLATFORM_UNSUPPORTED
    assert registry.route("linux", platform="linux", max_cost=1).reason is ReasonCode.COST_LIMIT_EXCEEDED


def test_risk_and_nondeterminism_need_consent() -> None:
    registry = CapabilityRegistry([capability("danger", risk=Risk.HIGH), capability("random", determinism=Determinism.NONDETERMINISTIC)])
    ...
    assert registry.route("random").reason is ReasonCode.NONDETERMINISTIC_REQUIRES_CONSENT
...
def test_unhealthy_candidate_returns_consent_required_repair_plan() -> None:
    item = capability("offline", health=Health.UNHEALTHY, health_detail="missing binary", repair_actions=(RepairAction.REINSTALL,))
...
def test_session_pin_prevents_silent_switching() -> None:
    registry = CapabilityRegistry([capability("first", fallback=("second",)), capability("second")])
...
def test_session_pin_is_stable_across_requested_alias() -> None:
    ...
    assert registry.route("second", session_id="s1").reason is ReasonCode.PINNED_CAPABILITY
