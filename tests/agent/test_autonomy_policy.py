...
def _action(risk=ActionRisk.READ, *, mutating=False):
    return ActionRequest("sha256:action", "sha256:goal", "workspace:one", risk, mutating)
...
def test_supervised_mode_asks_for_mutations_but_allows_read():
    ...
    assert policy.decide(_action(), now_ns=10).kind is PolicyDecisionKind.ALLOW
    assert policy.decide(_action(ActionRisk.REVERSIBLE_WRITE, mutating=True), now_ns=10).kind is PolicyDecisionKind.ASK


def test_approval_is_bound_to_digest_goal_scope_policy_and_expiry():
    policy = AutonomyPolicy(level=AutonomyLevel.L3_GOAL_SCOPED)
    approval = ApprovalGrant("sha256:action", "sha256:goal", "workspace:one", 20, policy.policy_version)
    action = _action(ActionRisk.PAYMENT, mutating=True)
    assert policy.decide(action, now_ns=19, approval=approval).kind is PolicyDecisionKind.ALLOW
    assert policy.decide(action, now_ns=21, approval=approval).reason is PolicyReason.HUMAN_GATE_REQUIRED
    other = ActionRequest("sha256:other", action.goal_hash, action.scope, action.risk, action.mutating)
    assert policy.decide(other, now_ns=19, approval=approval).kind is PolicyDecisionKind.ASK


def test_killswitch_denies_even_a_valid_approval():
    policy = AutonomyPolicy(level=AutonomyLevel.L4_PERSISTENT)
    approval = ApprovalGrant("sha256:action", "sha256:goal", "workspace:one", 20, policy.policy_version)
    decision = policy.with_killswitch().decide(_action(ActionRisk.DELETE, mutating=True), now_ns=10, approval=approval)
...
def test_l0_and_disallowed_risk_fail_closed():
    ...
    assert restricted.decide(_action(ActionRisk.INSTALL), now_ns=1).reason is PolicyReason.RISK_NOT_ALLOWED
