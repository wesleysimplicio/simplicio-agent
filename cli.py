...
def load_cli_config() -> Dict[str, Any]:
    ...
    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``simplicio-agent chat --ignore-user-config``),
    ...
    # `simplicio-agent config`/`doctor`/guards (which use load_config) honor it. The
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``simplicio-agent --help`` paths inside the chat command flow).  See
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.hermes/hermes-agent`` clone is updated only by ``simplicio-agent update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run simplicio-agent -w")
        return None
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    These are auto-generated by ``simplicio-agent -w`` sessions and PR review
...
# ASCII Art - SIMPLICIO AGENT logo (verde + amarelo)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "Simplicio Agent"
        tiny_line = "Simplicio"
    else:
        agent_name = _skin.get_branding("agent_name", "Simplicio Agent") if _skin else "Simplicio Agent"
        line1 = agent_name
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Simplicio Agent v{_version} ({_release_date})"
    else:
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # by `simplicio-agent chat --ignore-rules` in hermes_cli/main.py. When true we
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `simplicio-agent update` to rebuild the venv) to restore persistence."
                )
            except Exception:
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Simplicio")
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``simplicio-agent doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``simplicio-agent doctor`` so the banner stays
        small.
        """
        try:
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'simplicio-agent setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Run `simplicio-agent` to chat; `hermes` remains available as a legacy alias.{_RST}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``simplicio-agent sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `simplicio-agent sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``simplicio-agent --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: simplicio-agent --resume {self.session_id}")
        except Exception as e:
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `simplicio-agent model` and gateway pickers).
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                # Discover from disk (bundled + user), matching `simplicio-agent plugins
                ...
                # catalog lives behind `simplicio-agent plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: simplicio-agent plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_hermes_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: simplicio-agent plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: simplicio-agent plugins list)")
                    print("  Enable/disable: simplicio-agent plugins enable/disable <name>")
            except Exception as e:
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if msg_count > 0:
            ...
            print(f"  simplicio-agent --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  simplicio-agent -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Simplicio Agent")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Simplicio Agent"
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Simplicio Agent")
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: simplicio-agent setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `simplicio-agent sessions list`
...
def main(
    ...
    """
    Simplicio Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Simplicio Gateway...")
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `simplicio-agent skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``simplicio-agent chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                # Single-query mode (`simplicio-agent chat -q "…"`): skip the welcome
