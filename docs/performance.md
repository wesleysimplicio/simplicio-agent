...
| `rust_ext/`, `agent/_hermes_fast.py` | `hermes_fast` native extension importable | pure-Python fallback in the same module | build with `maturin` (`dev` extra includes it), or `pip install` the prebuilt `hermes_fast-*.whl` manylinux x86_64 wheel attached to GitHub Releases (`.github/workflows/release.yml`, issue #113 — other platforms still need a manual `maturin` build for now); optionally set `HERMES_RUST_ESTIMATES=1` to prefer Rust estimates where both paths exist |
