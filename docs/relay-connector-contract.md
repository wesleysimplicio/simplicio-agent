...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `simplicio-agent gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`simplicio-agent gateway enroll` → connector `/relay/enroll`, or
