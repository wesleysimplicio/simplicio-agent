...
def entropy_bits(x: int) -> float:
    """Shannon entropy proxy (bits) for a non-negative integer's min encoding.

    H = log2(x+1). Used to assert H(f(x)) == H(x): the bijection preserves the
    information content (0 loss), it does not compress below entropy.
    """
...
def selftest():
    ...
    for addr in addrs:
        ...
        # 2) entropy invariant under the bijection (0 loss)
        assert abs(entropy_bits(v) - entropy_bits(int(s, 16))) < 1e-9, \
            f"entropy not invariant at {addr}"
