...
## Command specialization and absorbed subskills

This umbrella skill is the primary entrypoint for Firecrawl work. Narrow command-specific skills have been absorbed into support references under `references/` so discoverability stays centered on one class-level workflow instead of many near-duplicate siblings.

Use those references when you need the distinctive details for:
- `search`
- `scrape`
- `map`
- `crawl`
...
- start at the highest-leverage step in the escalation chain
- do not split skill discovery across one skill per subcommand unless the user truly needs implementation detail
- keep file outputs and credit usage explicit
