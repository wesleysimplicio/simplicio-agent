...
- A long-running process (simplicio-agent gateway, tui_gateway) misbehaves and you can't restart it
...
### `tui_gateway` subprocess (spawned by `simplicio-agent --tui`)
...
debugpy.wait_for_client()
```
Start `simplicio-agent --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
