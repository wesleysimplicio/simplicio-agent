...
## Platform pitfalls surfaced in validation

- **macOS tempdir is not always `/tmp`.** When a repo bug only reproduces inside temp workspaces,
  remember that `tempfile.gettempdir()` and pytest `tmp_path` often resolve under
  `/private/var/folders/...`. If a file-write safety guard treats all `/private/var/...` paths as
  system-sensitive, validation can fail before the intended behavior is exercised. Compare against
  the real resolved temp root, not a hard-coded `/tmp` assumption. See
  `references/macos-tempdir-sensitive-path.md`.
