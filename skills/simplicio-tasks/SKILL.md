...
>
> **O banco neural continua sendo a primeira consulta** quando isso ajuda a evitar retrabalho:
> ```bash
> simplicio memory query "<problema>"    # verificar se já sabe
> simplicio skills recall "<tarefa>"     # verificar skill existente
> ```
>
...
| repo-root cleanup / ignore-rule hygiene for session artifacts | `references/root-artifact-hygiene.md` |
| `references/fallback-routing-and-issue-batching.md` | issue batching, fan-out, and honest fallback routing |
| **multi-repo batch git** — commit+push across several repos in parallel | `references/multi-repo-batch-git.md` |
| **external absorption** — research external repos & port code via parallel agents | `references/external-absorption.md` |

### Multi-repo external absorption rule (Jesse/Asolaria-style work)
When the user asks to absorb "the best of" an external ecosystem across MANY local repos, do **two fan-outs in parallel**:
1. **repo-by-repo fan-out** — one worker per local repository so no project is forgotten;
2. **cluster-by-cluster fan-out** — separate workers for the external pattern families (for example: federation/BEHCS, ai-memory/terminal topology, claims-gate/scoring/watcher verification).

Then integrate the findings into a **three-column honest matrix per repo**:
- **already exists**
- **applied / in progress**
- **still missing**

Never compress this class of work into a single "optimized everything" claim. For this workflow, completion requires explicit evidence that each local repo was checked, plus a final statement of which repos still lack real absorption.

### Continuity rule for Wesley-style autonomous runs
When the user says to **continue without stopping** (`"sempre continue"`, `"implemente tudo sem parar"`, `"não faça um por vez"`), treat that as a workflow contract for the whole class of task:
- do **not** pause after a partial success just to give a status update;
- as soon as one validation batch finishes, dispatch the **next parallel wave** immediately;
- keep the answer focused on **what is done, what is still blocked, and what is already running next**;
- never imply global completion from partial repo coverage.

### Simplicio MCP-first, CLI-fallback truth rule
If the user asks to use Simplicio MCP, try the MCP surface first. But when the current Hermes session does **not** expose Simplicio MCP tools, do **not** stall or keep asking:
1. state plainly that MCP is not exposed in this session;
2. switch immediately to the verified CLI fallback (`~/.local/bin/simplicio ...`);
3. continue the same multi-repo loop with Simplicio as the execution layer;
4. report the fallback honestly as an execution detail, **not** as a blocker.
