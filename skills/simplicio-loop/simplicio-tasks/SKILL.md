...
| external-contributions (parallel PR pipelines, pitfall catalog) | `references/hermes-external-contributions.md` |
| open GitHub issues with AC + parallel runtime execution | `references/gh-issues-ac-parallel-execution.md` |
| honest backlog drain (quarantine, don't fake-close) | `references/honest-backlog-drain.md` · `scripts/issue_repro_probe.py` |
...
---
iteration: 1
max_iterations: <backstop: max(10, 3×item-count); or 0 only when a $ budget ceiling is set>
...
**Latest workflow extraction:** see `references/latest-loop-workflow-notes.md` for the concise
background/parallel/stop-hook/iteration requirements distilled from the current docs review.
...
**Issues + AC + parallel exec:** when the user asks to open GitHub issues with acceptance criteria and run work in parallel, follow `references/gh-issues-ac-parallel-execution.md` (use `--body-file` for `gh issue comment`, not inline `-b` with backticks).
...
## Correlated release DAG — upstream first, downstream second
When the scope spans correlated repositories/packages (for example mapper → dev-cli → loop), do NOT fan out implementation, merges, and releases as if all lanes were independent. The dependency order is part of the acceptance criteria.

1. Discover and deduplicate first: query live issue/PR/release state. A merged PR may still leave an issue open, and an already-released version may predate that merge; neither is proof that a new release is unnecessary.
2. Freeze the dependency graph in the scratchpad: upstream package release → downstream dependency update → downstream release → next downstream update. Keep downstream PR preparation allowed only if useful, but block downstream merges/releases until the upstream release is live and verified.
3. Finish the upstream release gate: synchronize all version sources, changelog, lockfiles, package metadata, release PR, focused/static validation, tag/release publication, and a live package/tag query. Do not let a downstream consumer pin a version that is not yet published.
4. Update the immediate consumer to the verified upstream version, then repeat the same PR/validation/release gate. Continue transitively.
5. Run the final cross-repository tests only after the last release in the chain. Re-query issues and close-gate only after merged PR + evidence + live issue state. Create an aggregate/new release only when the explicit zero-issue/release condition is true.
6. Infrastructure blockers (billing lock, unavailable CI, missing artifact publication) are blockers for release completion, not evidence to bypass gates. Prepare PRs, label the state `UNVERIFIED|`/blocked, and never fabricate a published release.

Detailed recipe and live-state checks: `references/correlated-release-dag.md`.

## Ordered backlog waves and honest delivery gate

When the scope is a multi-repository backlog with known live issue counts, **sort lanes by ascending open-issue count before starting execution**. Start the smallest repository, verify that its lane is genuinely running/completed, then advance to the next count; preserve deterministic tie-breaking (repository name) for equal counts. Do not launch every repository indiscriminately when the user requests an order.

A lane result such as `completed_fixture`, `planned`, `queued`, or `throttled` is **not** proof that an issue was delivered. Count an item as delivered only after a real PR/commit artifact, passing evidence gate, and live source re-query confirm the outcome. If the runtime reports fixture/simulation work, label it as fixture evidence and keep the source issue open; never summarize it as completed. Record runtime throttling separately from successful work and continue with the next safe lane only when the ordering contract permits it.

For the reusable recipe and the observed runtime result taxonomy, see `references/ordered-backlog-wave.md`.

## Self-correction gate after every wave/worker result

A process exit code of 0 proves only that the orchestrator returned normally. It does **not** prove implementation, PR creation, merge, or issue closure. After every lane, wave, or factory run, classify the result from live artifacts before advancing:

1. `completed_fixture`, `planned`, `queued`, `queued_throttled`, `throttled`, or `dry_run` are orchestration/fixture states, never delivery states. Keep the source issue open and label the result `UNVERIFIED|`.
2. A delivered item requires all of: real changed files/commit, PR URL and live head, passing focused evidence, merged PR on the intended base, and live issue re-query showing `closed`. `auto_merge_allowed: false` means no merge occurred unless a separate live query proves it.
3. Confirm the actual loop was armed and exercised: scratchpad, journal, watcher state, attempt records, and an evidence-gated promise/close-gate. A command that merely mentions `simplicio-loop` or a fixture issue title is not loop execution.
4. If a lane falls back to fixture/throttle, stop broad fan-out, inspect the runtime preflight/contract smoke and adapter resolution, repair the root cause through `simplicio edit --plan`, rebuild the runtime when required, and rerun the smallest lane first. Do not compensate for a broken preflight by launching more workers.
5. For ordered backlog waves, sort by ascending live open-issue count (ties by repository name), start one smallest lane, verify it is genuinely running/completed, then advance. Never launch all repositories concurrently when ordering is part of the goal.
6. The final promise is forbidden until live GitHub queries prove PR/merge/close state. Persist a handoff with the exact blocker and next command when the cap or environment prevents completion.

This gate exists because a successful scheduler/fixture wrapper previously looked like a completed backlog drain while producing no PRs, no merges, and no closed issues.
...
- **Heavy-path** (large queue OR any medium+ item): fan out a CONTINUOUS WORKER POOL fed by a LIVE
  ...
  **Worktree mechanics + abort/cleanup protocol** — read `references/worktree-cleanup-abort.md`
  BEFORE any mass parallel worktree run. Covers: PATH-FIRST ordering (`git worktree add "$WT"
  "branch"`), the branch-already-exists trap (drop `-b`, reuse the local branch), the
  `pkill`-before-`worktree remove` rule so subagents/background shells stop recreating worktrees,
  orphan `issue/*` branch cleanup, and the lost-cwd fix. Getting the abort order wrong costs 4+
  cleanup passes.
...
### Parallel-PR mode (for bulk mechanical refactors)
When the queue contains multiple independent mechanical changes (idiom fixes, import cleanup, docs patches) across non-overlapping files, fan them out via `delegate_task` with one item per agent rather than implementing serially. Each agent: branch → patch → commit → push → PR. Keep batches small (3-5 files per agent). Verify each agent returned a PR number — partial failures are common.

> ⚠️ **CRITICAL:** Never create duplicate PRs. Before every batch, run `gh search prs --repo <target> --author <your-org> --state open` AND check the issue's timeline for linked PRs. A duplicate wastes everyones time and damages contributor reputation. Close it immediately if found.
...
> ⚠️ **HONEST DRAIN — never bulk-close by repro (Wesley, 2026-07-11).** Draining a large issue queue does NOT mean `gh issue close` on every item. A command cited in an issue returning `rc=0` is close-evidence ONLY for a **specific bug report** whose exact failing command now works. Epics / feature requests / roadmap items with no testable AC must be **quarantined** (recorded in the journal, left open) until a real PR delivers them. In one session, auto-closing 19 items as "stale" because one sub-command worked closed 15 epics falsely — caught and reopened before the run ended. The loop's close-gate requires PR-merged + in-turn evidence + live re-query confirming `state=closed`; a bare `gh issue close` with no delivered AC is the parcial the user forbade. See `references/honest-backlog-drain.md`.
...
## Runtime-evolution token savings
Use `references/runtime-evolution-token-savings.md` for session-learned guidance on compact maps,
evidence receipts, deterministic edits, and parallel fan-out. For issue queues under a scope freeze,
see `references/issue-triage-scope-freeze.md`.

- Default repo maps should be source-only; exclude generated trees unless explicitly requested.
- For repeat turns, prefer delta maps keyed by changed files and direct dependents.
- Evidence should be carried by stable receipt IDs and artifact handles, not re-read logs.
- If the change is already decided, route it through deterministic edit/apply paths.
- Fan out independent lanes; serialize only same-file or same-contract work.
